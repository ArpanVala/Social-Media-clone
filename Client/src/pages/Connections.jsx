  ...
  return (
    <div className='max-h-screen'>
      <div className='max-w-6xl mx-auto p-6'>
          {/* title  */}
        <div className='mb-8'>
          <h1 className='text-2xl font-semibold text-title '>Connections</h1>
          <h6 className='text-lg text-subtitle'>Manage your network and discover new connections</h6>
        </div>
        ...
        {/* followers, following etc counts  */}
          ...
          {/* Tabs  */}
          <div className='inline-flex flex-wrap gap-3  items-center border border-mute rounded p-1 bg-plain shadow'>
          {
            dataArray.map((tab, idx) => (
              ...
              >
                ...
                {tab.count !== undefined && (
                  <span className='ml-2 text-xs bg-background mute-3 px-2 py-0.5 rounded-full'>
                    {tab.count}
                  </span>)}
              </button>
            ))
          ...
          <div className='flex flex-wrap gap-6 mt-8'>
          {
            dataArray.find((item)=> item.label === currentTab)?.value.map((user) => (
              <div className='bg-plain shadow-md rounded-md max-w-88 flex gap-5 w-full p-5' key={user._id}>
                <img src={user.profile_picture} alt="profile picture" className='w-12 h-12 rounded-full object-cover shadow-md ' />
                <div className='flex-1 text-sm'>
                  <p className='font-semibold text-subtitle'>{user.full_name}</p>
                  <p className='font-light text-subtitle'>@{user.username}</p>
                  <p className='font-light text-subtitle'>{user.bio.slice(0, 50)}...</p>

                  <div className='flex max-sm:flex-col gap-2 mt-4'>
                {
                <button className='w-full p-2 text-sm bg-gradient-to-r rounded-md from-accent to-gradient-end-hover  text-plain transition active:scale-95 cursor-pointer' 
